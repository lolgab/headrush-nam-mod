 ...
 * mirroring scripts/quickstart_mac.sh's own behavior. Not part of
 * nam_gui_core (it shells out to `cp`/`codesign`, both always present on
 * macOS but not portable) -- app-layer glue only, used by app/main.c.
 */
 ...
 * case-insensitive, same as `find "$APP" -iname Update.img` in
 * quickstart_mac.sh). `app_dir` is the *.app directory itself (see
 ...
 * ad-hoc codesigns it (`codesign --force --deep --sign -`) -- matching
 * quickstart_mac.sh, a codesign failure is logged via `err` but does NOT
 * fail the overall operation (still returns true): an unsigned app is a
 * normal, expected outcome for any unofficial build here. */
