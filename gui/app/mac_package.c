...
/* Case-insensitive recursive search for a file named "Update.img" --
 * matches `find "$APP" -iname Update.img` in quickstart_mac.sh. */
                          ...
                          const char* output_app_path, char* err, size_t err_size)
{
  ...
  if (system(cmd) != 0)
  {
    /* Matches quickstart_mac.sh: a failed ad-hoc re-sign is a warning, not
     * a hard failure -- usually harmless for a non-quarantined local copy. */
