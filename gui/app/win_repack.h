 ...
 * port of scripts/repack_windows_updater.py -- see that file's docstring
 * for the full format rationale (PE stub + `;!@Install@!...;!@InstallEnd@!`
 * config block + a plain 7z archive containing Background.png,
 * Config.json, Update.img, FirmwareUpdater.exe, libusb-1.0.dll).
 *
 ...
 * extract/create steps, same as repack_windows_updater.py itself does.
 ...
 * See repack_windows_updater.py's strip_authenticode_directory() for why:
 ...
 * unchanged entry) repack_windows_updater.py does. `sevenzip_path` is the
