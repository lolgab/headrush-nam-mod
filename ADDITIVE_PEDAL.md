...
This is the writeup for the second NAM pedal design mentioned in the
README: a genuinely new "Neural Amp Modeler" pedal type, sacrificing
nothing. It is **not applied by `build_update_img.py`** — kept here as
reverse-engineering notes for whoever picks up the reachability problem.
...
Because of both, `build_update_img.py` does not invoke
...
whatever index the real UI reserves), re-wire `patch_namloader.py`'s call
back into `build_update_img.py`, and re-validate under QEMU before trying
on real hardware.
