 ...
 * writer, C port + extension of the old Python build pipeline's read-only
 * FIT reader (it only read; it handed the .its text to the real `mkimage`
 * binary to write). This port additionally WRITES a spec-compliant FIT
 * blob directly -- see fit_image.c's header comment for
